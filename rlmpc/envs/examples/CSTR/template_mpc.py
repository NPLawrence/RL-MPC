...
def template_mpc(model, vf = None, goal = 0.6, mpc_mode = "nstep_vfmpc", n_horizon = 1, silence_solver = True, solver="pardiso",
    ...
    if uncertain_params == "nominal": 
        ...
        alpha_var, beta_var = [np.array([1.05, 0.95]), np.array([1.1, 0.9])]
    ...
    if mpc_mode == "baseline" or mpc_mode == "nominal":
        mpc.settings.n_horizon = 20 # hardcoded bc this is the original value
        if mpc_mode == "nominal":
        ...
        mpc.scaling['_x', 'T_R'] = 100
        mpc.scaling['_x', 'T_K'] = 100
        mpc.scaling['_u', 'Q_dot'] = 2
        mpc.scaling['_u', 'F'] = 10

        mterm = -np.exp(-0.5*(model.aux['track_error']/ 0.25)**2)
        lterm = -np.exp(-0.5*(model.aux['track_error']/ 0.25)**2)

        mpc.set_objective(mterm=mterm, lterm=lterm)

        mpc.set_nl_cons('T_R', model.x['T_R'], ub=140, soft_constraint=True)
        ...
        lterm = -np.exp(-0.5*(model.aux['track_error']/ 0.1)**2)
        # call l4casadi
        lf_l4c = l4c.L4CasADi(vf, model_expects_batch_dim=False, device='cpu')
        z = vertcat(model.x.cat, model.tvp['goal'] - model.x.cat[1])
        mterm = -lf_l4c(z.T)

        mpc.set_objective(mterm=mterm, lterm=lterm)

        mpc.set_nl_cons('C_b', model.x['C_b'], ub=2, soft_constraint=True)
        mpc.set_nl_cons('C_a', model.x['C_a'], ub=2, soft_constraint=True)
        mpc.set_nl_cons('T_K', model.x['T_K'], ub=140, soft_constraint=True)
        mpc.set_nl_cons('T_R', model.x['T_R'], ub=140, soft_constraint=True)
        ...
        # An early special case of nstep_vfmpc here for archival purposes: maximize the critic without n-step predictions
        ...
        # regular l4casadi
        lf_l4c = l4c.L4CasADi(vf, model_expects_batch_dim=False, device='cpu')
        z = vertcat(model.x.cat, model.tvp['goal'] - model.x.cat[1], model.u.cat)
        lterm = -lf_l4c(z.T)

        mpc.set_objective(mterm=mterm, lterm=lterm)
        # mpc.set_rterm(F = 0.1, Q_dot = 0.1)
